...
/*
  ...
  datafile : root datafile to be passed to readData() this file is used for 
             the fit if genParams is "".
 ...
 */
void systematicFitter(int nToys, TString genParams, TString fitParams,
		      //TString weightDataSet, 
		      TString outFilename,
		      bool randomizeTail, int binning = 1, 
		      TString datafile = "MuOnia.root") {
  TStopwatch timer;
  timer.Start();
  ...
  buildPdf(oniaWS);
  readData(oniaWS, datafile);
  RooRealVar * invariantMass = oniaWS.var("invariantMass");
  RooAbsPdf * pdf = oniaWS.pdf("pdf");

  // RooRealVar acc1("acc1","acc1", 1.0);
  // RooRealVar acc2("acc2","acc2", 1.0);
  // RooRealVar acc3("acc3","acc3", 1.0);
  // RooRealVar effWgt("effWgt","effWgt", 1.0);

  RooRealVar * upsPt = oniaWS.var("upsPt");

  // RooArgList dataCols(*invariantMass, acc1, acc2, acc3, effWgt, Ypt);
  RooDataSet * data = 0;
  data = dynamic_cast<RooDataSet *>(oniaWS.data("data"));

  RooThresholdCategory ptbint("ptbin", "ptbin", *upsPt);
  double * ptBinArr = ptBinArr1S;
  int nbins = sizeof(ptBinArr1S)/sizeof(double);
  if (binning == 2) {
    ptBinArr = ptBinArr2S;
    nbins = sizeof(ptBinArr2S)/sizeof(double);
  } else if (binning == 3) {
    ptBinArr = ptBinArr3S;
    nbins = sizeof(ptBinArr3S)/sizeof(double);
  }
  char buf[100];
  ...
  RooCategory ptbin(*(dynamic_cast<RooCategory *>(data->addColumn(ptbint))));
