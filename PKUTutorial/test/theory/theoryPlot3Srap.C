theoryPlot3Srap(){
  ...
  TFile cmsFile("xsection_3s_y0.root");
...
//  TGraphAsymmErrors* cmsdata1 = ((TMultiGraph*)cmsFile.Get("mg1"))->GetListOfGraphs()->At(1);
  ...
  TH1F *frame = gPad->DrawFrame(0,0.001,2,50);
  //TH2F * frame = new TH2F("frame","", 1000,0.,30., 0, 0.001,10);
  frame->SetStats(0);

  frame->GetXaxis()->SetTitle("|y^{#Upsilon}|");
  TString ytitle = TString::Format("d#sigma/d|y|* BR(#Upsilon(3S)#rightarrow#mu#mu)  (nb/(GeV/c))");
  ...
  TFile* fp = new TFile("../GenCrossSection/genXSupsilon3Srap.root"); // open the file
...
//  cmsdata1->Draw("Psame");
  ...
  TLatex latex3;
  latex3.DrawLatex(0.2,0.003 , "#sqrt{s}=7 TeV, L= 3 pb^{-1}");
...
//  leg->AddEntry(gg, "CASCADE", "L");
  ...
  leg->Draw();
  c2->Print("theoryPlot3Srap.gif");
  c2->Print("theoryPlot3Srap.eps");
  c2->Print("theoryPlot3Srap.pdf");

  hist1->Scale(1.04/1.7);
  hist1->Draw("sameHIST");
  c2->Print("theoryPlot3SrapScaled.gif");
  c2->Print("theoryPlot3SrapScaled.eps");
  c2->Print("theoryPlot3SrapScaled.pdf");
}
