theoryPlot2Srap(){
  ...
  TFile cmsFile("xsection_2s_y0.root");
...
//  TH1F *frame = gPad->DrawFrame(0,0.001,2,10);
  //TH2F * frame = new TH2F("frame","", 1000,0.,30., 0, 0.001,10);
  frame->SetStats(0);

  frame->GetXaxis()->SetTitle("|y^{#Upsilon}|");
  TString ytitle = TString::Format("d#sigma/d|y|* BR(#Upsilon(2S)#rightarrow#mu#mu)  (nb/(GeV/c))");
...
//////////////////////////////////////////////
  ...
  TFile* fp = new TFile("../GenCrossSection/genXSupsilon2Srap.root"); // open the file
...
//  gg->Draw("same" "HIST");
  ...
  TLatex latex3;
  latex3.DrawLatex(0.1,0.2 , "#sqrt{s}=7 TeV, L= 3 pb^{-1}");

  TLegend *leg = new TLegend(0.56,0.7,0.89,0.9);
  leg->SetBorderSize(0);
  leg->SetFillColor(10);
...
//  leg->AddEntry(gg, "CASCADE", "L");
  ...
  leg->Draw();
  c2->Print("theoryPlot2Srap.gif");
  c2->Print("theoryPlot2Srap.eps");
  c2->Print("theoryPlot2Srap.pdf");

  hist1->Scale(1.94/6.47);
  hist1->Draw("sameHIST");
//  cmsdata->Draw("P");
//  cmsdata1->Draw("Psame");
  c2->Print("theoryPlot2SrapScaled.gif");
  c2->Print("theoryPlot2SrapScaled.eps");
  c2->Print("theoryPlot2SrapScaled.pdf");
}
