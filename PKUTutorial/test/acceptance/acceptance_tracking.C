...
void acceptance_tracking(const char* file = "tree_tracking_mixed.root", const char* outputName = "acceptance_tracking.root") {
  gROOT->Reset();
  ...
  if(i % 1000 == 0) printf("event %d\n", i);
   TLorentzVector genUps; genUps.SetPtEtaPhiM(genUpsPt[0], genUpsEta[0], genUpsPhi[0], 9.46);
    ...
    Float_t alpha = 0;//0,-1,1,-0.34,0.34
  ...
  if ( recoMuSize ==2 && ((recoMuPt[0] >= 3.5 && fabs(recoMuEta[0]) < 1.2) || (recoMuPt[0] >= 3.0 && fabs(recoMuEta[0]) < 2.4 &&  fabs(recoMuEta[0]) > 1.2)) && ((recoMuPt[1] >= 3.5 && fabs(recoMuEta[1]) < 1.2) || (recoMuPt[1] >= 3.0 && fabs(recoMuEta[1]) < 2.4 &&  fabs(recoMuEta[1]) > 1.2))){
